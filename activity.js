...
const addEntry = (title, author) => {
    ...
    //const index = bookList.indexOf({ title: "The Great Gatsby", author: "F. Scott Fitzgerald" },);
    //bookList = bookList.splice(index, 1);
}
addEntry("FountainHead", "Ann Rand")



const removeEntry = (title, author) => {
    const index = bookList.findIndex((elem) => {
        return elem.title === title && elem.author === author
    });
    console.log(index)
